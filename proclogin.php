...
if (!isset($_SESSION['prev']) || ($_SESSION['prev'] != "login")) {header("Location: logout.php");exit;}
...
if (isset($_POST['problem'])) { // nori pagalbos
    $_SESSION['message'] = "Turi būti įvestas galiojantis vartotojo vardas";} else { $_SESSION['message'] = "Pabandykite dar kartą";}
...
{list($dbuname, $dbpass, $dblevel, $dbuid, $dbemail) = checkdb($user); //patikrinam ir jei randam, nuskaitom DB
    if ($dbuname) { //yra vartotojas DB
        ...
        if (checkpass($pass, $dbpass)) { // vardas ir slaptazodis geras
            if ($dblevel == UZBLOKUOTAS) {$_SESSION['message'] = "Jūsų paskyra užblokuota";
                $_SESSION['name_error'] =
                ...
                if (!$yra) {$_SESSION['message'] = "Negaliojanti vartotojo rolė.";
                    $_SESSION['name_error'] =
                        "<font size=\"2\" color=\"#ff0000\">* Prisijungimas negalimas. Kreipkitės į administratorių</font>";} else {
                    //prijungiam
