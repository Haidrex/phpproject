...
if (!isset($_SESSION['prev']) || (($_SESSION['prev'] != "admin") && ($_SESSION['prev'] != "procadmin"))) {header("Location: logout.php");exit;}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($naikinti || ($nlevel != $level)) {$keisti[] = $user; // cia isiminti kuriuos keiciam, ka keiciam bus irasyta i $pakeitimai
        echo "<tr><td>" . $user . "</td><td>"; // rodyti sia eilute patvirtinimui
        if ($level == UZBLOKUOTAS) {
            echo "Užblokuotas";
        } else {foreach ($user_roles as $x => $x_value) {if ($x_value == $level) {
            echo $x;
        }
        }
        }
        echo "</td><td>";
        if ($naikinti) {echo "<font color=red>PAŠALINTI</color>";
            $pakeitimai[] = -1; // ir isiminti  kad salinam
            $naikpoz = true;
        } else { $pakeitimai[] = $nlevel; // isiminti i kokia role
            if ($nlevel == UZBLOKUOTAS) {
                echo "UŽBLOKUOTAS";
            } else {foreach ($user_roles as $x => $x_value) {if ($x_value == $nlevel) {
                echo $x;
            }
            }
            }
